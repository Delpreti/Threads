...
void init_matrix(int* ma, int* mb, int lines, int cols){
    ...
    for(w = 0; w < (pow(2, lines - 1) - 1); w++){
        for(v = 0; v < pow(2, current_line); v++){
            int aux = pow(2, current_line - 1);
            ma[u + v] = mb[u - cols + aux - 1];
            mb[u + v] = pow(2, current_line) + v + (2 * (u % cols));
        }
        u += v;
        current_line = u / cols;
    }
...
void* thread_function(void* args_pointer){
    ...
    for(int i = args->thread_index; i < limit; i += args->total_threads){
